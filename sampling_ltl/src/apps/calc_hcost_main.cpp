...
// ---- Prototype the get_lifted_graph functions ----
// Where does it go?
double get_lifted_transition(int H, std::vector<Vertex<SquareCell>> tile_vertices_Vertex, std::vector<int> rgn_idx_next, std::map<int, std::shared_ptr<Hlevel>>& Hlevels, int N_REGION_TOTAL){
	...
	std::shared_ptr<Tile> this_tile_data = std::make_shared<Tile>(H,tile_vertices);
	...
	Eigen::Matrix<int,rgn_idx_next.size(),N_REGION_TOTAL + 1> connect_block;
	int m = 0;
	...
	Eigen::RowVectorXi sum_row_binary = sum_row.array() > 0;
	...
	if (sum_row_binary.sum()){
		...
		rgn_idx_next = RowVectorXi::Map(indices.data(),indices.size());
	}else{
		cost = 1.0e9;
		rgn_idx_next.clear();
	}
...
// Initial condition function for zta0
int zta02rgn_idx(vector<double> zta0){ // zta0 should be 2 element vector [location, angle]
	int rgn_idx_init;
...
//			map = std::get<1>(sg_map);
	...
	/*** otherwise, create a square grid map manually ***/
		...
		// create a empty grid
		grid = std::make_shared<SquareGrid>(5,5,1);
