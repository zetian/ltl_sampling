...
double LTL_Sampling3d::get_dist(std::vector<double> states_1, std::vector<double> states_2) {
    double dist = sqrt(pow(states_1[0] - states_2[0], 2) + pow(states_1[1] - states_2[1], 2) + pow(states_1[2] - states_2[2], 2));
    return dist;
}

double LTL_Sampling3d::fRand(double fMin, double fMax)
{
...
std::vector<int> LTL_Sampling3d::sample_from_ba(BAStruct buchi, SampleSpace &sample_space)
{
...
void LTL_Sampling3d::buchi_post (BAStruct &ba, std::vector<int> indep_set) {
...
bool LTL_Sampling3d::if_in_region (std::vector<double> state, Region region) {
...
&& state[2] > region.get_z_position().first && state[2] < region.get_z_position().second) {
        return true;
    }
    else {
        return false;
...
std::vector<double> LTL_Sampling3d::step_from_to (SampleNode parent_sample, std::vector<double> sampled_state) {
    double length = get_dist(parent_sample.get_state(), sampled_state);
    if (length < EPSILON_) {
        return sampled_state;
    }
    else {
        double theta_1 = acos((sampled_state[0] - parent_sample.get_state()[0])/length);
        double theta_2 = acos((sampled_state[1] - parent_sample.get_state()[1])/length);
        double theta_3 = acos((sampled_state[2] - parent_sample.get_state()[2])/length);
        // double theta_1 = atan2(sampled_state[1] - parent_sample.get_state()[1], sampled_state[0] - parent_sample.get_state()[0]);
        // double theta_2 = atan2(sampled_state[1] - parent_sample.get_state()[1], sampled_state[0] - parent_sample.get_state()[0]);
        std::vector<double> new_state = {parent_sample.get_state()[0] + (EPSILON_)*cos(theta_1),
            parent_sample.get_state()[1] + (EPSILON_)*cos(theta_2),
            parent_sample.get_state()[2] + (EPSILON_)*cos(theta_3)};
        return new_state;
    }
}
...
void LTL_Sampling3d::read_formula(std::string ltl_formula, std::vector<std::string> buchi_regions, std::vector<int> indep_set) {
    SpotHoaInterpreter ltl2ba_lib;
    ba_ = ltl2ba_lib.GetBuchi(ltl_formula, buchi_regions);
    buchi_post(ba_, indep_set);
}
void LTL_Sampling3d::init_workspace(double work_space_size_x, double work_space_size_y, double work_space_size_z) {
    work_space_size_x_ = work_space_size_x;
    work_space_size_y_ = work_space_size_y;
    work_space_size_z_ = work_space_size_z;
}

void LTL_Sampling3d::init_parameter(double EPSILON, double RADIUS){
    EPSILON_ = EPSILON;
    RADIUS_ = RADIUS;
}

std::vector<double> LTL_Sampling3d::sample_state(std::vector<int> ba_act) {
    double new_node_x = 0;
    double new_node_y = 0;
    double new_node_z = 0;
    if (ba_act[0] == ba_act[1]) {
        ...
        new_node_z = fRand(0, work_space_size_z_);
    }
    else {
        int interest_id = 0;
        ...
        double new_node_z_min = all_interest_regions_.find(interest_id)->second.get_z_position().first;
        double new_node_z_max = all_interest_regions_.find(interest_id)->second.get_z_position().second;
        new_node_x = fRand(new_node_x_min, new_node_x_max);
        new_node_y = fRand(new_node_y_min, new_node_y_max);
        new_node_z = fRand(new_node_z_min, new_node_z_max);
    }
    std::vector<double> sampled_position = {new_node_x, new_node_y, new_node_z}; 
    return sampled_position;
}

void LTL_Sampling3d::set_interest_region(std::pair <double, double> position_x, std::pair <double, double> position_y, std::pair <double, double> position_z, int interest_id) {
    Region interest_region;
    interest_region.set_position(position_x, position_y, position_z);
    interest_region.set_region_interest(interest_id);
    all_interest_regions_[interest_id] = interest_region;
}

void LTL_Sampling3d::set_obstacle(std::pair <double, double> position_x, std::pair <double, double> position_y, std::pair <double, double> position_z){
    Region obstacle;
    obstacle.set_position(position_x, position_y, position_z);
    all_obstacles_.push_back(obstacle);
}

void LTL_Sampling3d::set_init_state(std::vector<double> init_state) {
...
void LTL_Sampling3d::start_sampling(int iteration) {
    ...
    for (int i = 0; i < iteration; i++) {
        ...
        // node_data.state[2] = new_sample_state[2];
...
std::vector<std::vector<double>> LTL_Sampling3d::get_path() {
    bool find_path = false;
